...
![gp timeline](/media/golden-path-diagram-selection-V2.png)

Blue steps are efficiency plays: they save time and reduce risk. Red steps are growth plays: they turn the same governed foundation into revenue. Notice that growth never shows up before step 6. You cannot monetize or externalize what you have not first governed.
...
| 5. Governed coding agents | **WSO2 Agent Manager** , enterprise control plane for AI agents. **WSO2 Agent Builder**. **WSO2 AI Gateway**. **WSO2 API Portal & MCP Hub**. **WSO2 Developer Platform / OpenChoreo** for golden paths, CI gates, and environment promotion enforced by the platform rather than by convention. |
| 6. Expose capabilities | **WSO2 Agent Manager** . **WSO2 Agent Builder**. **WSO2 AI Gateway**. **WSO2 API Portal & MCP Hub**. **WSO2 Developer Platform / OpenChoreo**. |
